"""
Management command: django_waf_sync_feed
...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser) -> None:
        parser.add_argument(
            ...
            type=str,
            default=None,
            help="Override the feed URL (default: DJANGO_WAF_FEED_URL).",
        )
        parser.add_argument(
            "--min-confidence",
            type=float,
    ...
    def handle(self, *args, **options) -> None:
        ...
        if not conf.DJANGO_WAF_FEED_ENABLED and not feed_url:
            self.stdout.write(
                self.style.WARNING("DJANGO_WAF_FEED_ENABLED is False and no --feed-url was supplied. Skipping sync.")
            )
            return
