"""
Management command: django_waf_install_geoip
...
Usage::
    ...
    # One-off install (licence key from DJANGO_WAF_MAXMIND_LICENSE_KEY):
    manage.py django_waf_install_geoip

    # Pass the licence key explicitly:
    manage.py django_waf_install_geoip --license-key=XXXXXXXXXXXXXXXX

    # Skip if existing file is younger than 7 days (cron-friendly):
    manage.py django_waf_install_geoip --if-older-than=7 --quiet

    # Override output path:
    manage.py django_waf_install_geoip --output-path=/etc/geoip/country.mmdb
"""
...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser) -> None:
        parser.add_argument(
            "--license-key",
            default=None,
            help="MaxMind licence key. Defaults to DJANGO_WAF_MAXMIND_LICENSE_KEY.",
        )
        parser.add_argument(
            "--output-path",
            default=None,
            help="Destination .mmdb path. Defaults to DJANGO_WAF_GEOIP_PATH.",
        )
        parser.add_argument(
            "--if-older-than",
            type=int,
    ...
    def handle(self, *args, **options) -> None:
        from django_waf.services.geoip import (
            GeoIPDownloadError,
            GeoIPError,
