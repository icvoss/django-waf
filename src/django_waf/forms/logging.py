...
from django_waf.forms.signals import (
...
def log_form_submission(*, form_id: str, request, result: FormEvaluationResult) -> None:
    ...
    Sampling:
      * PASSED → ``DJANGO_WAF_LOG_SAMPLE_RATE``
      * FLAGGED / BLOCKED → always logged (sampling N/A)

    Signal emission:
      * PASSED  → form_submission_passed iff DJANGO_WAF_FORM_EMIT_PASSED_SIGNAL
...
def _maybe_emit_passed_signal(*, form_id: str, ip: str, user_agent: str, user_id: str | None) -> None:
    ...
    if not conf.DJANGO_WAF_FORM_EMIT_PASSED_SIGNAL:
        return
    try:
        ...
        logger.exception("django-waf: form_submission_passed receiver raised")
...
def _dispatch_signal(
    *,
    ...
    try:
        ...
        logger.exception("django-waf: form-submission signal receiver raised")
