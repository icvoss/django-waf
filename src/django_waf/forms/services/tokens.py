...
def get_signing_key() -> bytes:
    ...
    1. ``DJANGO_WAF_SIGNING_KEY`` if non-empty.
    ...
    The fallback is documented and surfaced via the ``django_waf.W003``
    ...
    if conf.DJANGO_WAF_SIGNING_KEY:
        return conf.DJANGO_WAF_SIGNING_KEY.encode("utf-8")

    # Fallback: namespace-derive from SECRET_KEY so we never use the raw
    # secret directly. ``b"django-waf:signing:v1"`` is the namespace; bumping
    ...
    return hashlib.sha256(b"django-waf:signing:v1|" + raw).digest()
