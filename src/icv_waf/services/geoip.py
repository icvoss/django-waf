"""
GeoIP database management for icv-waf.
...
is read from ``ICV_WAF_MAXMIND_LICENSE_KEY`` or passed explicitly.
...
logger = logging.getLogger("icv_waf.geoip")
...
DEFAULT_OUTPUT_PATH = "/var/lib/icv-waf/GeoLite2-Country.mmdb"
...
def resolve_license_key(explicit: str | None = None) -> str:
    ...
    key = getattr(conf, "ICV_WAF_MAXMIND_LICENSE_KEY", "")
    if not key:
        raise GeoIPLicenseMissingError(
            "No MaxMind licence key configured. Sign up at "
            f"{SIGNUP_URL} and set ICV_WAF_MAXMIND_LICENSE_KEY in "
...
def resolve_output_path(explicit: str | None = None) -> Path:
    ...
    path = explicit or getattr(conf, "ICV_WAF_GEOIP_PATH", None) or DEFAULT_OUTPUT_PATH
    return Path(path)
...
def install_geoip_database(
    ...
    Args:
        license_key: MaxMind licence key. Defaults to ``ICV_WAF_MAXMIND_LICENSE_KEY``.
        output_path: Destination ``.mmdb`` path. Defaults to ``ICV_WAF_GEOIP_PATH``
            or ``/var/lib/icv-waf/GeoLite2-Country.mmdb``.
    ...
    if if_older_than_days > 0 and is_database_fresh(dest_path, if_older_than_days):
        logger.info(
            "icv-waf: GeoIP database at %s is younger than %d days — skipping download",
            dest_path,
            if_older_than_days,
        )
        return {
    ...
    with tempfile.TemporaryDirectory(prefix="icv-waf-geoip-") as tmpdir:
    ...
    logger.info(
        "icv-waf: GeoIP database %s installed at %s (%d bytes)",
        edition,
        dest_path,
        size,
    )
        ...
        with httpx.stream("GET", url, timeout=60, follow_redirects=True) as response:
            if response.status_code == 401:
                raise GeoIPDownloadError(
                    ...
                    f"Check ICV_WAF_MAXMIND_LICENSE_KEY or sign up at {SIGNUP_URL}."
                )
