...
Evaluation order under test (BR-EVAL-003):
  1. DJANGO_WAF_ENABLED master kill switch
...
class TestExemptHosts:
    ...
    @override_settings(
        ...
        DJANGO_WAF_EXEMPT_HOSTS=["admin.example.com"],
    ...
    @override_settings(
        DJANGO_WAF_ENABLED=True,
        DJANGO_WAF_EXEMPT_HOSTS=[".example.com"],
...
class TestMiddlewareHelpers:
    """Tests for the small helper functions in django_waf.middleware.
    ...
    def test_compute_fingerprint_delegates_to_service(self):
        from django_waf.middleware import _compute_fingerprint
    ...
    def test_classify_fingerprint_delegates_to_service(self):
        from django_waf.middleware import _classify_fingerprint
    ...
    def test_lookup_country_no_geoip_path_returns_empty(self):
        """Without DJANGO_WAF_GEOIP_PATH configured, _lookup_country returns ''."""
        from django_waf.middleware import _lookup_country

        with patch("django_waf.conf.DJANGO_WAF_GEOIP_PATH", ""):
    ...
    def test_lookup_country_db_missing_returns_empty(self):
        ...
        import django_waf.services.geoip as geoip_module
        from django_waf.middleware import _lookup_country
        ...
        with patch("django_waf.conf.DJANGO_WAF_GEOIP_PATH", "/nonexistent/GeoLite2-Country.mmdb"):
            assert _lookup_country("1.2.3.4") == ""
    ...
    def test_lookup_country_returns_iso_code(self):
        ...
        try:
            with patch("django_waf.conf.DJANGO_WAF_GEOIP_PATH", "/tmp/fake.mmdb"):
    ...
    def test_emit_request_blocked_swallows_exception(self):
        ...
        with patch(
            "django_waf.signals.request_blocked.send",
...
class TestGetChallengePaths:
    def test_setting_overrides_reverse(self):
        """DJANGO_WAF_CHALLENGE_URL / _VERIFY_URL short-circuit reverse()."""
        import django_waf.conf as conf_mod

        middleware = _make_middleware()
        with (
            patch.object(conf_mod, "DJANGO_WAF_CHALLENGE_URL", "/custom/challenge/"),
            patch.object(conf_mod, "DJANGO_WAF_VERIFY_URL", "/custom/verify/"),
            patch("django_waf.middleware.reverse") as mock_reverse,
        ):
    ...
    def test_falls_back_to_reverse_when_unset(self):
        ...
        middleware = _make_middleware()
        with (
            patch.object(conf_mod, "DJANGO_WAF_CHALLENGE_URL", ""),
            patch.object(conf_mod, "DJANGO_WAF_VERIFY_URL", ""),
            patch(
                "django_waf.middleware.reverse",
