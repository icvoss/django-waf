...
**Service domain:** threats.drystane.com
**Status:** Design. Superseded in part by `docs/specs/django-waf/06-threat-feed-api.md`
(the published wire contract) and ADR-021, which name threats.drystane.com as the
reference operated server; this document's server-implementation detail below
predates that decision and is retained as background, not as a live status report.
**Date:** April 2026
...
(`build_telemetry_payload` / `submit_telemetry`). This document specifies the
central service design; see `docs/specs/django-waf/06-threat-feed-api.md` for the
authoritative, versioned wire contract the client actually implements.
...
the central service should be published at `threats.drystane.com/privacy`.
...
Base URL: `https://threats.drystane.com`
...
| **threats.drystane.com** | /24 CIDRs, UA hashes | Django-specific bots | Free/paid | Native, automatic |
...
- [ ] A real installation can set `DJANGO_WAF_FEED_REPORT = True` and
  ...
  from `threats.drystane.com/v1/report`
...
- [ ] `GET https://threats.drystane.com/v1/feed.json` returns a valid JSON feed with
